...
{
public:
	// auto operator[](TAirport port) {
	// 	return
	// }
	// конструктор по умолчанию: список элементов пока пуст
	AirportCounter()
	{
		size_t n = static_cast<size_t>(TAirport::Last_);
		for (size_t i = 0; i < n; i++)
		{
			list[i].first = static_cast<TAirport>(i);
		}
	}
	...
	{
		return list[static_cast<size_t>(airport)].second;
	}

	// добавить данный элемент
	void Insert(TAirport airport)
	{
		size_t id = static_cast<size_t>(airport);
		auto *obj = &list[id];
		obj->second++;
	}
